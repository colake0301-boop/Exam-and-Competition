...
\begin{frame}{Arzela-Ascoli: Sequential Version}
    \textbf{Context:} 
    In finite dimensions ($\mathbb{R}^n$), the Bolzano-Weierstrass theorem tells us that every bounded sequence has a convergent subsequence. In infinite-dimensional function spaces, boundedness is not enough.

    \vspace{1em}

    \textbf{Theorem (Sequential):} 
    Let $X$ be a compact metric space. If a sequence $\{f_n\} \subset C(X, \mathbb{R})$ is:
    \begin{enumerate}
        \item \textbf{Uniformly Bounded} ($\exists M, \forall n, x: |f_n(x)| \le M$), and
        \item \textbf{Equicontinuous},
    \end{enumerate}
    then $\{f_n\}$ contains a \textbf{uniformly convergent subsequence}.

    \vspace{1em}
    
    \textbf{Key Concept:} 
    A set where every sequence has a convergent subsequence (whose limit might be outside the set) is called \textbf{Relatively Compact}.
\end{frame}

\begin{frame}{Arzela-Ascoli: Topological Characterization}
    \textbf{Transition:} 
    To characterize \textit{Compact} sets (sets that contain their limit points), we combine relative compactness with the "Closed" property.

    \vspace{0.5em}

    \textbf{Theorem (Topological):} 
    Let $X$ be a compact metric space. A subset $K \subseteq C(X, \mathbb{R})$ is \textbf{compact} in the uniform topology if and only if $K$ is:
    \begin{enumerate}
        \item \textbf{Closed} (contains all its limit points),
        \item \textbf{Uniformly Bounded}*, and
        \item \textbf{Equicontinuous}.
    \end{enumerate}

    \vspace{0.5em}
    {\footnotesize \textit{*Note: On a compact domain $X$, Pointwise Boundedness + Equicontinuity $\implies$ Uniform Boundedness.}}

    \vspace{1em}
    
    \textbf{Intuition:} 
    This is the infinite-dimensional analog of the \textbf{Heine-Borel Theorem}. In $\mathbb{R}^n$, Closed + Bounded $\iff$ Compact. In $C(X, \mathbb{R})$, we must add "Equicontinuity" to handle the infinite dimensions.
\end{frame}
...
\begin{frame}{Problem 17: Diagonal Argument Lemma}
    \begin{problem}
        Let $X$ be a metric space with a countable dense subset $S = \{x_1, x_2, \dots \}$. Let $\{f_n\}$ be a sequence of functions in $C(X, \R)$ that is uniformly bounded. Show that there exists a subsequence $\{f_{n_k}\}$ that converges pointwise on $S$.
    \end{problem}
    \textit{This is the first step in the proof of the Arzela-Ascoli Theorem.}
\end{frame}

\begin{frame}{Solution to Problem 17: Diagonal Argument}
    \begin{proof}
        \begin{enumerate}
            \item Consider the sequence evaluated at $x_1$: $\{f_n(x_1)\}$. Since $\{f_n\}$ is uniformly bounded, this scalar sequence is bounded. By Bolzano-Weierstrass, there is a subsequence $\{f_{1,k}\}$ converging at $x_1$.
            \item From $\{f_{1,k}\}$, extract a subsequence $\{f_{2,k}\}$ converging at $x_2$. Note it still converges at $x_1$.
            \item Inductively, construct subsequence $\{f_{m,k}\}$ converging at $x_1, \dots, x_m$.
            \item \textbf{Diagonal Sequence:} Let $g_k = f_{k,k}$.
            \item For any fixed $x_m \in S$, the sequence $\{g_k\}$ eventually becomes a subsequence of $\{f_{m,k}\}$ (for $k \ge m$), so it converges at $x_m$.
            \item Thus, $\{g_k\}$ converges pointwise on $S$.
        \end{enumerate}
    \end{proof}
\end{frame}
...
\begin{frame}{Problem 18: Removing Lines from $\R^2$}
    \begin{problem}
...
\begin{frame}{Solution to Problem 18 (Nested Balls)}
    \begin{proof}
...
\begin{frame}{Problem 19: Generic Nowhere Differentiability}
    \begin{problem}
...
\begin{frame}{Solution to Problem 19 (1/4): Setup}
...
\begin{frame}{Solution to Problem 19 (2/4): Closedness}
...
\begin{frame}{Solution to Problem 19 (3/4): Nowhere Dense}
...
\begin{frame}{Solution to Problem 19 (4/4): Conclusion}
...
\begin{frame}{Problem 20: Generic Nowhere Monotonicity}
    \begin{problem}
...
\begin{frame}{Solution to Problem 20 (1/2): Setup \& Closedness}
...
\begin{frame}{Solution to Problem 20 (2/2): Nowhere Dense \& Conclusion}
...
\begin{frame}{Problem 21: Continuity on Dense Subsets}
    \begin{problem}
        Suppose $f: \R \to \R$ is continuous on a dense subset $D \subseteq \R$. Show that the set of all discontinuity points of $f$ must be of the \textbf{First Category}.
    \end{problem}
    \textit{Note: As a consequence of BCT, we cannot have a function continuous precisely on $\Q$ and discontinuous on $\R \setminus \Q$, because $\R \setminus \Q$ is of Second Category.}
\end{frame}

\begin{frame}{Solution to Problem 21}
    \begin{proof}
        Let $C(f)$ be the set of continuity points and $D(f)$ be the set of discontinuity points.
        Recall that $D(f)$ is always an $F_\sigma$ set (countable union of closed sets).
        $$ D(f) = \bigcup_{n=1}^\infty \{ x \mid \omega_f(x) \ge 1/n \} $$
        where $\omega_f(x)$ is the oscillation of $f$ at $x$. The sets $A_n = \{ x \mid \omega_f(x) \ge 1/n \}$ are closed.
        
        We are given that $D \subseteq C(f)$ is dense in $\R$.
        Since $C(f)$ is dense, $D(f)$ has empty interior (if it contained an interval, that interval would contain points of $D$, a contradiction).
        Thus, each closed set $A_n \subseteq D(f)$ has empty interior, making $A_n$ nowhere dense.
        Therefore, $D(f) = \bigcup A_n$ is a countable union of nowhere dense sets (First Category).
    \end{proof}
\end{frame}

\begin{frame}{Problem 22: Pointwise Limits (Baire Class 1)}
    \begin{problem}
        Suppose $f_n: \R \to \R$ is a sequence of continuous functions, and $f(x) = \lim_{n \to \infty} f_n(x)$ exists for all $x$. Show that the set of discontinuity points of $f$ is of the \textbf{First Category}.
    \end{problem}
    \textit{This means $f$ is continuous on a dense, Second Category set (almost continuous).}
\end{frame}

\begin{frame}{Solution to Problem 22}
    \begin{proof}
        This is the \textbf{Baire Classification Theorem}. A pointwise limit of continuous functions is a Baire Class 1 function.
        For such functions, the set of points of discontinuity $D(f)$ is always of the First Category.
        
        \textbf{Sketch:}
        Define the oscillation $\omega_f(x)$. We check sets $F_k = \{x \mid \omega_f(x) \ge 1/k\}$.
        We can express these sets using the continuity of $f_n$.
        It can be shown that $D(f)$ is an $F_\sigma$ set of first category.
        Specifically, Baire proved that for Baire-1 functions, the set of continuity points $C(f)$ is a dense $G_\delta$ set.
        Since $\R$ is complete, a dense $G_\delta$ set is Second Category (comeager), so $D(f)$ is First Category (meager).
    \end{proof}
\end{frame}
...
\begin{frame}{Problem 23: Taylor Series}
    \begin{problem}
...
\begin{frame}{Solution to Problem 23}
...
\begin{frame}{Problem 24: IFT in $\R^2 \times \R^2$}
    \begin{problem}
...
\begin{frame}{Solution to Problem 24 (1/2): Statement}
...
\begin{frame}{Solution to Problem 24 (2/2): Derivative Formula Proof}
...
\begin{frame}{Problem 25: IFT Calculation}
    \begin{problem}
        Consider the system:
...
\begin{frame}{Solution to Problem 25: Calculation}
...
\begin{frame}{Problem 26: Feynman's Trick Practice}
    \begin{problem}
...
\begin{frame}{Solution to Problem 26 (1/4): Part (a) — Setup}
\textbf{Technique: Feynman Trick (Differentiation under the Integral Sign)}

Define the parameter-dependent integral
\[
F(t) := \int_0^1 \frac{x^t - 1}{\ln x} \, dx, \qquad t>0.
\]

Observe that the required integral can be written as
\[
I = \int_0^1 \frac{x^b - x^a}{\ln x} \, dx = F(b) - F(a).
\]
...
\begin{frame}{Solution to Problem 26 (2/4): Part (a) — Evaluation}

Integrate $F'(t)$ with respect to $t$:
\[
F(t) = \int \frac{1}{t+1} \, dt = \ln(t+1) + C.
\]

Determine the constant using $F(0)$:
\[
F(0) = \int_0^1 \frac{1-1}{\ln x} \, dx = 0
\quad \Rightarrow \quad C=0.
\]
...
\begin{frame}{Solution to Problem 26 (3/4): Part (b) — Setup}
\textbf{Technique: Feynman Trick with a Parameter}
...
\begin{frame}{Solution to Problem 26 (4/4): Part (b) — Evaluation}
...
= \frac{1}{\alpha^2+1}
\left[
\int_0^1 \frac{dx}{1+\alpha x}
- \int_0^1 \frac{\alpha - x}{1+x^2} \, dx
\right].
\]

Compute each integral:
\[
\int_0^1 \frac{dx}{1+\alpha x} = \frac{1}{\alpha}\ln(1+\alpha),
\]
\[
\int_0^1 \frac{dx}{1+x^2} = \frac{\pi}{4},
\qquad
\int_0^1 \frac{x}{1+x^2} dx = \frac12 \ln 2.
\]
...
Integrating from $0$ to $1$ and using $F(0)=0$, we obtain
\[
\boxed{
I = F(1) = \frac{\pi}{8}\ln 2
}
\]
\end{frame}
...
\begin{frame}{Problem 27: Improper Integrals with Parameters}
    \begin{problem}
        Compute:
        \begin{enumerate}
...
\begin{frame}{Solution to Problem 27 (1/2): Part (a)}
...
\begin{frame}{Solution to Problem 27 (2/2): Part (b)}
...
\begin{frame}
    \centering
    \Large Good luck with the Final!
\end{frame}
